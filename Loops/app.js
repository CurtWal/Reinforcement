...
const eggs = [81, 75, 75, 81, 32, 75, 81, 75, 81];
  ...
  console.log(`The eggs array is ${eggs.length} items long`);

  // 22. How many 32s are in array eggs?
  let numTotal = eggs.filter(egg => egg === 32);
  if (numTotal.length < 32) {
    console.log(`There are ${numTotal.length} 32(s) in this array`);
  }

  // 23. Does array eggs have only 75s and 81s?
  let combinedTotal = eggs.filter(egg => egg === 75 || egg === 81);
  if (combinedTotal.length < 75) {
    console.log(`There are ${combinedTotal.length} 75(s) and 81(s) in the eggs array`);
  }
...
// get the animation id childen from the html doc
const frames = document.getElementById("animation").children;

// let frameCount equal the amount of childen in the animation id 
